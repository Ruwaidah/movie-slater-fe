...
export const login = (loginData) => async (dispatch) => {
  ...
  await axiosWithAuth()
    ...
    .then((response) => {
      ...
      dispatch({ type: USER_LOGING_IN_SUCCESS, payload: response.data.user });
    });
  // .catch(err => {
  //   console.log(err)
...
export const getUserById = () => async (dispatch) => {
  ...
  if (path) {
    ...
    dispatch({ type: USER_BYID_FAILURE, payload: "no user Id" });
  }
};
...
export const signUpGoogle = (signUpData) => async (dispatch) => {
  ...
  await axiosWithAuth()
    ...
    .then((response) => {
      ...
      localStorage.setItem("token", response.data.token);
      return dispatch({
        type: USER_SIGNING_GOOGLE_SUCCESS,
  ...
  await axiosWithAuth()
    ...
    .then((respone) => {
      ...
      if (theatres.length == 0)
        dispatch({
          type: GET_SHOWTIMES_RESULTS_SUCCESS,
          payload: [respone.data, theatres],
        });
      else {
        axiosWithAuth()
          ...
          .then((data1) => {
            dispatch({
              type: GET_SHOWTIMES_RESULTS_SUCCESS,
              payload: [respone.data, data1.data],
            });
          })
          .catch((err) =>
            dispatch({
              type: GET_SHOWTIMES_RESULTS_FAILED,
              payload: err.respone,
            })
          );
      }
...
export const delfavoriteTheatres = (id) => (dispatch) => {
  let path;
  if (localStorage.getItem("googleId")) path = localStorage.getItem("googleId");
  else if (localStorage.getItem("userId"))
